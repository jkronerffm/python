...
class Time(Text):
    def __init__(self):
        super().__init__("", pos=Point((400, 200)),font = Fonts.Arial128, color = Colors.Blue, orientation = Orientation.BottomCenter)
...
class Date(Text):
    def __init__(self):
        super().__init__("", pos=Point((400, 200)), font = Fonts.Arial64, color = Colors.Blue, orientation = Orientation.TopCenter)
...
class Clock(GraphObjectGroup):
    def __init__(self, size = None, pos = Point((0, 0)), orientation = Orientation.TopLeft, active=True):
        ...
        super().__init__(pos=pos, size=size, orientation=orientation, backgroundColor = Colors.Black, alpha=32, active = active)
...
if __name__ == "__main__":
    ...
    try:
        runner = PGRunner(Size((1024, 768)), Colors.White)
